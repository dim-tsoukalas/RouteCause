...
Ties the two layers together, in the K8sGPT spirit:
  ...
  2. For each finding, retrieve cited RFC grounding via the CitationEngine.
...
is constrained to cite retrieved sources or abstain.
"""
...
class InvestigationEngine:
    ...
    def investigate(self, incident: Incident, question: str) -> Report:
        ...
        # Layer 2: cited grounding. Build a retrieval query from the question
        # plus the finding hints, so the explanation is anchored to what was
        # actually observed.
        hints = [
            f.rfc_hint
            for r in results
        ...
        if hints:
            retrieval_query = f"{question} ({'; '.join(dict.fromkeys(hints))})"
        report.explanation = self.citations.query(retrieval_query)

        report.next_steps = self._next_steps(results)
        return report

    def ask(self, question: str):
        """Doc-only Q&A path (the `ask` verb) — retrieval + citation, no analyzers."""
        return self.citations.query(question)
